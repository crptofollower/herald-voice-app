  ...
  try {
    ...
    assert('production ChatScreen supplies an explicit reminiscenceArcRef at every processUtterance call',
      processCalls.length === 2
        ...
        && chatSrc.includes('conversationLedgerRef.current, reminiscenceArcRef.current);')
        && (chatSrc.match(/conversationLedgerRef\.current, reminiscenceArcRef\.current\)/g) ?? []).length === 2,
      (v) => v === true, 'true');

...
export async function runConversationTurnLedgerCoverageTests() {
  ...
  // hotNarrativeRing.test.ts already uses for its own SEAM assertions.
  {
    ...
    assertTrue('ChatScreen: processUtterance call threads the ledger through', /discourseRef\.current, conversationLedgerRef\.current, reminiscenceArcRef\.current\)/.test(chatSrc));
    assertTrue(

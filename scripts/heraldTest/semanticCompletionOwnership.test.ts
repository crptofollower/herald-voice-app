...
export async function runSemanticCompletionOwnershipTests() {
  ...
  resetSemanticCompletionLifecycleForTests({ recoveryHorizonMs: 40 });
  {
    ...
    d.resolve({ text: '{"ok":true}' });
    await wait(10);
    assert('7 onAcquired fires once and does not prove late-result discard',
      proposalCalls === 1, (v) => v === true, 'once');
  ...
  resetSemanticCompletionLifecycleForTests();
  {
    ...
    d.resolve({ text: JSON.stringify({ disposition: 'AUTOBIOGRAPHICAL', confidence: 0.9 }) });
    await wait(20);
    const events = peekSemanticCompletionBreadcrumbs().map((b) => b.event);
    assert('6 recollection consumer stays timeout and receives no late proposal',
      gen.status === 'unavailable' && gen.reason === 'timeout' && !('disposition' in gen),
      (v) => v === true, 'unavailable timeout');
    assert('6 late_result_discarded is recorded for the abandoned recollection',
      events.includes('late_result_discarded'), (v) => v === true, 'discarded');
    assert('6 abandoned recollection does not emit native_settled',
      !events.includes('native_settled'), (v) => v === true, 'no native_settled');
  }
  ...
  {
    const first = deferred();
    const done = runSharedSemanticCompletion(() => first.ctx, { n: 1 });
    await wait(5);
    first.resolve({ text: '{"ok":true}' });
    await done;
    await wait(5);
    const refused = await runSharedSemanticCompletion(undefined, { n: 2 });
    const requested = [...peekSemanticCompletionBreadcrumbs()].reverse().find((b) => b.event === 'semantic_requested');
    assert('released operation id is not reused by the next request',
      refused.status === 'unavailable' && refused.reason === 'no_ctx'
      && requested?.operation == null && requested?.generation == null,
      (v) => v === true, 'null operation');
  }

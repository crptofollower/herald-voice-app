...
class HeraldMedicalTemporalContinuityTest {
  companion object {
    private const val TAG = "HeraldMedicalTemporal"
  ...
  @Test
  fun medicalTemporalContinuity_packagedSendMessage() {
    ...
    for (s in 0 until scenarios.length()) {
      ...
      val reset = JSONObject(HeraldJourneyBridge.resetScenario(RESET_TIMEOUT_MS, scenarioId))
  ...
  private fun loadContract(): JSONObject {
    ...
    ctx.assets.open("herald/android.medicalTemporal.v1.scenarios.json").use { stream ->
